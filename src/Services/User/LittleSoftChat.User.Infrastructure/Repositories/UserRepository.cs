...
public class UserRepository : IUserRepository
{
    ...
    public async Task<UserEntity?> GetByUsernameAsync(string username)
    {
        ...
        return await connection.QuerySingleOrDefaultAsync<UserEntity>(sql, new { Username = username });
    }
    ...
    public async Task<List<UserEntity>> GetByIdsAsync(IEnumerable<int> ids)
    {
        using var connection = _dbFactory.CreateConnection();
        ...
        var users = await connection.QueryAsync<UserEntity>(sql, new { Ids = ids });
        return users.ToList();
    }

    public async Task<List<UserEntity>> SearchUsersAsync(string keyword)
    {
        using var connection = _dbFactory.CreateConnection();
        ...
        var users = await connection.QueryAsync<UserEntity>(sql, new { Keyword = $"%{keyword}%" });
        return users.ToList();
    }
