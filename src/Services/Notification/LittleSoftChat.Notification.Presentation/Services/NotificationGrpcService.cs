...
namespace LittleSoftChat.Notification.Presentation.Services;
...
public class NotificationGrpcService : NotificationService.NotificationServiceBase
{
    ...
    public override async Task<NotificationResponse> MarkAllAsRead(MarkAllAsReadRequest request, ServerCallContext context)
    {
        try
        {
            ...
            {
                Success = result,
                ErrorMessage = result ? "" : "Failed to mark all notifications as read"
            };
        }
        catch (Exception ex)
        {
            _logger.LogError(ex, "Error in MarkAllAsRead gRPC call");
            return new NotificationResponse
            {
                Success = false,
                ErrorMessage = "Internal server error"
